...
CROSSCOMPILE = yes
NOX11 = yes
CFLAGS += -Wall -pipe -O2 -DNDEBUG -fomit-frame-pointer
...
LINK_CMD = $(QUIET_LINK) $(CC) $(LDFLAGS) -o $@ $^ $(LIBS) && $(STRIP) -s $@
...
PDF_APPS := $(addprefix $(OUT)/, pdfops pdfdraw pdfclean pdfextract pdfinfo pdfshow)
